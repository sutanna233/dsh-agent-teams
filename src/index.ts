...
2. Call agent_teams_add_member once per role the goal needs (researcher, engineer, reviewer, ...). Members are durable subagents: they wait for your messages, then work a full turn. You decide each member's model yourself: by default a member inherits your current provider/model/reasoning effort, and you may pass provider/model to route a member to a different model when the task warrants it (a cheaper model for mechanical chores, a stronger one for hard reasoning). Review the model inspection tools available in your toolset to see registered models when choosing. Never ask the user to choose a model.
